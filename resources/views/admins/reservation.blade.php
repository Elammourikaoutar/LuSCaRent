...
<html lang="en">
  ...
  <div class="dropdown mt-3">
    ...
    <div class="dropdown mt-6 active">
    <button class="btn dropdown-toggle active"style="background-color: #a38840;" type="button" id="user-dropdown" data-bs-toggle="dropdown">
      Admin
    </button>
    <ul class="dropdown-menu"aria-labellebdy="user-dropdown">
    <li >
    ...
    <li class="active">
      <a class="dropdown-item active" style="font-size: 18px;"href="{{route('admins.reservation')}}">
        Commands
      </a>
    </li>
    <li>
...
<div class="card">
            <div class="card-body">
                <table class="table">
                    ...
                    <tbody>
                    @foreach($commands as $command)                       <tr>
                       <td>
                        <img src="{{asset($command->avatar)}}"
                        width="80"
                        height="80"
                        class="img-fluid"
                        alt="" srcset="">
                       </td>
                       <td>{{App\Models\user::findorFail($command->user_id)->name}}</td>
                       <td>{{App\Models\user::findorFail($command->user_id)->email}}</td>
                       <td>{{App\Models\user::findorFail($command->user_id)->tel}}</td>
                       <!-- <td>{{App\Models\user::findorFail($command->user_id)->notes}}</td> -->
                       ...
                       <td>{{$command->prixTTC}}</td>
                           ...
                           <td  class="d-flex flex-row justify-content-center">
                            <!-- <a href="{{route('commands.edit',[$command->id,$command->car_id])}}" method="post" class="btn btn-warning mr-2" ><i class="fa fa-edit"></i></a> -->
                            <form action="{{route('commands.sup',[$command->id,$command->car_id])}}" method="post">
                           @csrf
                           {{method_field('delete')}}
